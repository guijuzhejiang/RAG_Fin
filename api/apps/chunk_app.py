...
def list_chunk():
    ...
    try:
        ...
        sres = retrievaler.search(query, search.index_name(tenant_id), highlight=True, query_search=False)
    ...
    try:
        ...
        if doc.parser_id == ParserType.QA:
            ...
            d = beAdoc(d, arr[0], arr[1], not any(
                [rag_tokenizer.is_japanese(t) for t in q + a]))
...
def retrieval_test():
    ...
    page = int(req.get("page", 1))
    size = int(req.get("size", 30))
    search_depth = float(req.get("search_depth", 0.2))
    ...
    try:
        ...
        retr = retrievaler
        ranks = retr.retrieval(question, embd_mdl, kb.tenant_id, kb_id, page, size, search_depth,
...
def knowledge_graph():
    ...
    sres = retrievaler.search(req, search.index_name(tenant_id), query_search=True)
    obj = {"graph": {}, "mind_map": {}}
