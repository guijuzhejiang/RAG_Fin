...
def set_api_key():
    ...
    chat_passed, embd_passed, rerank_passed, vlm_passed = False, False, False, False
        ...
        if not embd_passed and llm.model_type == LLMType.EMBEDDING.value:
            ...
            try:
                ...
                msg += f"\nFail to access model({llm.llm_name}) using this api key." + str(
                    e)
            rerank_passed = True
        elif not vlm_passed and llm.model_type == LLMType.IMAGE2TEXT:
            mdl = CvModel[factory](
                req["api_key"], llm.llm_name, base_url=req.get("base_url"))
            try:
                path = "/home/zzg/商业项目/upwork/RAG/imgs/AOC_Coatings_page5.png"
                with open(path, "rb") as f:
                    file_bytes = f.read()
                arr, tc = mdl.describe(file_bytes)
                if len(arr) == 0 or tc == 0:
                    raise Exception("Fail")
            except Exception as e:
                msg += f"\nFail to access model({llm.llm_name}) using this api key." + str(
