...
from api.settings import RetCode, GITHUB_OAUTH, FEISHU_OAUTH, CHAT_MDL, EMBEDDING_MDL, ASR_MDL, IMAGE2TEXT_MDL, PARSERS, \
    API_KEY, \
    LLM_FACTORY, LLM_BASE_URL, RERANK_MDL, default_llm
...
def user_register(user_id, user):
    ...
    if LLM_FACTORY == 'Ollama':
        for model_type in ("embedding", "chat", "image2text"):
            tenant_llm.append({"tenant_id": user_id,
                               "llm_factory": LLM_FACTORY,
                               "llm_name": default_llm[LLM_FACTORY][f"{model_type}_model"],
