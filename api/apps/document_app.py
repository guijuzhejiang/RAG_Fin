...
def upload():
    ...
    for f_idx, file_obj in enumerate(file_objs):
        if file_obj.filename == '':
            ...
            if len(content) <= 0:
                return get_json_result(
                    data=False, retmsg=f'{file_obj.filename} is empty!', retcode=RetCode.DATA_ERROR)
            ...
            # 将内容解码为UTF-8并重新编码
            utf8_content = content.decode(encoding).encode('utf-8')

            # 创建一个新的BytesIO对象
            utf8_file = io.BytesIO(utf8_content)

            # 创建新的FileStorage对象
            new_file_storage = FileStorage(stream=utf8_file, filename=file_obj.filename, content_type=file_obj.content_type)

            file_objs[f_idx] = new_file_storage
