...
def rmPrefix(txt):
    return re.sub(
        r"^(问题|質問|答案|回答|user|assistant|Q|A|Question|Answer|问|答)[\t:： ]+", "", txt.strip(), flags=re.IGNORECASE)


def beAdocPdf(d, q, a, eng, image, poss):
    qprefix = "Question: " if eng else "質問："
...
def chunk(filename, binary=None, lang="Chinese", callback=None, **kwargs):
    ...
    if re.search(r"\.xlsx?$", filename, re.IGNORECASE):
        ...
        delimiter = "\t"
        csv_reader = csv.reader(StringIO(txt), delimiter=delimiter, quotechar='"', skipinitialspace=True)
