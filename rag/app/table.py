...
def chunk(filename, binary=None, from_page=0, to_page=10000000000,
    ...
    if re.search(r"\.xlsx?$", filename, re.IGNORECASE):
        ...
        lines = txt.split("\n")
        header_line = lines[0] # 读取文件的第一行
        if '\t' in header_line:
            sep = '\t'
        elif ',' in header_line:
            sep = ','
        else:
            raise ValueError("CSV文件中无法检测到合适的分隔符")

        df_csv = pd.read_csv(StringIO(txt), sep=sep, engine="python")

        callback(0.3, "Extract records: {}~{}".format(from_page, to_page))
        dfs = [df_csv]
    else:
    ...
    for df in dfs:
        ...
        py_clmns = [
            PY.get_pinyins(
                re.sub(r"(/.*|（[^（）]+?）|\([^()]+?\))", "", str(n)), '_')[0] for n in clmns]
        clmn_tys = []
