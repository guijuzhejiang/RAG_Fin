...
class FasterWhisperSeq2txt(Base):
    def __init__(self, key, model_name, lang="Japanese", **kwargs):
        self.url = kwargs["base_url"]
        self.lang = lang

    def transcription(self, audio_bin):
        #后端链接ASR docker container
        # 创建一个字典来表示表单数据
        # 定义请求头
        headers = {
        ...
        # 创建一个字典来表示表单数据
        files = {
            'audio_file': ('audio.wav', audio_bin, 'audio/wav')
        }
        ...
        if response.ok:
            return response.text, num_tokens_from_string(response.text)
        return "**ERROR**: No ASR results ", 0
