...
class DefaultRerank(Base):
        ...
        if not LIGHTEN and not DefaultRerank._model:
            ...
            with DefaultRerank._model_lock:
                if not DefaultRerank._model:
                    try:
                        model_path = os.path.join(get_home_cache_dir(), re.sub(r"^[a-zA-Z]+/", "", model_name))
    ...
    def similarity(self, query: str, texts: list):
        ...
        pairs = [(query, truncate(t, 512)) for t in texts]
        token_count = 0
        for _, t in pairs:
        ...
        for i in range(0, len(pairs), batch_size):
            # 使用较小的max_length值，避免内存问题
            scores = self._model.compute_score(pairs[i:i + batch_size], max_length=512)
