...
def collect():
    ...
    try:
        # 删除与队列相关的键值
        # REDIS_CONN.REDIS.xtrim(SVR_QUEUE_NAME, minid="0")
        # REDIS_CONN.REDIS.delete(CONSUMER_NAME)
        # REDIS_CONN.REDIS.delete(SVR_QUEUE_NAME)
...
def build(row, chat_mdl, vlm_mdl):
    ...
    try:
        cks = chunker.chunk(row["name"], binary=binary, from_page=row["from_page"],
                            ...
                            chat_mdl=chat_mdl, vlm_mdl=vlm_mdl)
        cron_logger.info("Chunking({}) {}/{}".format(timer() - st, row["location"], row["name"]))
    except Exception as e:
        callback(-1, "Internal server error while chunking: %s" % str(e).replace("'", ""))
        cron_logger.error("Chunking {}/{}: {}".format(row["location"], row["name"], str(e)))
        traceback.print_exc()
        return
...
def init_kb(row):
    ...
    return ELASTICSEARCH.createIdx(idxnm, conf.mapping)
...
def embedding(docs, mdl, parser_config=None, callback=None):
    ...
    cnts = cnts_
    #为了凸显文件名的作用，区分不同公司名称或年份，filename_embd_weight要修改的更大
    title_w = float(parser_config.get("filename_embd_weight", 0.3))
...
def main():
    ...
    for _, r in rows.iterrows():
        ...
        #加入vlm模型
        vlm_mdl = LLMBundle(r["tenant_id"], LLMType.IMAGE2TEXT, llm_name=r["img2txt_id"], lang=r["language"])
        ...
        if r.get("task_type", "") == "raptor":
            ...
            try:
                ...
                print("Embedding elapsed({}): {:.2f}".format(r["name"], timer() - st))
            except Exception as e:
        ...
        if es_r:
            ...
            try:
                print("clean cache")
                # 获取所有 keys，筛选出包含 tenant_id 的
                keys_to_delete = [key for key in REDIS_CONN.REDIS.keys('*') if str(r["tenant_id"]) in key]
                # 删除筛选出的 keys
                if keys_to_delete:
                    REDIS_CONN.REDIS.delete(*keys_to_delete)
...
if __name__ == "__main__":
    ...
    import deepdoc.vision.surya_ocr as surya_ocr

    surya_ocr.surya_model = surya_ocr.SuryaOCR()
    tables.load_agent()
