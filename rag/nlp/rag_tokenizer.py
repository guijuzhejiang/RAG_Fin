...
class RagTokenizer:
    def key_(self, line):
        ...
        # return line.lower().encode("utf-8").decode("utf-8")

    def rkey_(self, line):
        return str(("DD" + (line[::-1].lower())).encode("utf-8"))[2:-1]
        # return (("DD" + (line[::-1].lower())).encode("utf-8").decode("utf-8"))
        ...
        try:
            ...
            while True:
                ...
                try:
                    ...
                    F = int(math.log(value / self.DENOMINATOR) + .5)
    ...
    def __init__(self, debug=False):
        ...
        # self.DIR_ = os.path.join(get_project_base_directory(), "rag/res", "ja_huqie")
    ...
    def _tradi2simp(self, line, chinese=False):
        if chinese:
    ...
    #词形还原（lemmatization）和词干提取（stemming）,可能过度处理，导致语义失真,所以先不使用。
    ...
    def tokenize(self, line):
        ...
        if self.is_english(line):
            # 英文分词使用 NLTK word_tokenize
            tokens = word_tokenize(line)
            return " ".join(tokens)
        return line

    def fine_grained_tokenize(self, tks):
        if self.is_english(tks):
            # 英文不需要细粒度分词，直接返回
            return tks
        tks = tks.split(" ")
        zh_num = len([1 for c in tks if c and is_japanese(c[0])])
...
def is_japanese(s):
    #平假名、片假名和汉字
    if (s >= u'\u3040' and s <= u'\u309f') or (s >= u'\u30a0' and s <= u'\u30ff') or (s >= u'\u4e00' and s <= u'\u9fff'):
        return True
    else:
        return False
