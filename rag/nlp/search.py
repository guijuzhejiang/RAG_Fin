...
from rag.utils.print_sim import print_search_results_and_scores
...
class Dealer:
    ...
    class SearchResult:
        ...
        def from_dict(cls, dict_data):
            return cls(
                ...
                ids=dict_data['ids'],
                query_vector=dict_data['query_vector'],
                field=dict_data['field'],
                highlight=dict_data['highlight'],
                aggregation=dict_data['aggregation'],
                keywords=dict_data['keywords'],
                group_docs=dict_data['group_docs']
            )
    ...
    def search(self, req, idxnm, emb_mdl=None, highlight=False, query_search=True):
        qst = req.get("question", "")
        bqry, keywords = self.qryr.question(qst, min_match="60%")
        ...
        #                          "q_1024_vec", "q_1536_vec", "q_3584_vec", "available_int", "content_with_weight"])
        src = req.get("fields", ["docnm_kwd", "content_ltks", "kb_id", "img_id", "title_tks", "important_kwd",
                                 "image_id", "doc_id", "position_int", "knowledge_graph_kwd",
                                 "q_768_vec", "available_int", "content_with_weight"])
        if query_search:
            #知识库检索页面，生成条数在30-300条之间，当search_depth=0.1时，生成30条，当search_depth=1时，生成300条
            #对话页面，默认设置search_depth=0.05,生成条数=15
            search_depth_factor = 150
            # search_depth_factor = 100
            search_depth = req.get("search_depth", 0.2)
            search_depth = int(search_depth * search_depth_factor)
            s = s.query(bqry)[0:search_depth]
        else:
        ...
        for k in keywords:
            kwds.add(k)
            for kk in rag_tokenizer.fine_grained_tokenize(k):
                if len(kk) < 2:
                    continue
                if kk in kwds:
                    continue
    ...
    def retrieval(self, question, embd_mdl, tenant_id, kb_ids, page, page_size, search_depth=0.2, similarity_threshold=0.2,
                  vector_similarity_weight=0.3, top=1024, doc_ids=None, aggs=True, rerank_mdl=None, highlight=False):
        ranks = {"total": 0, "chunks": [], "doc_aggs": {}}
        if not question:
            return ranks
        req = {"kb_ids": kb_ids, "doc_ids": doc_ids, "search_depth": search_depth,
               "question": question, "vector": True,  "topk": top,
               "similarity": similarity_threshold,
               "available_int": 1}

        # 构造缓存的唯一key，例如基于用户查询和分页参数
        cache_key = f"{question}_{search_depth}_{tenant_id}_{'_'.join(kb_ids)}"
        # 检查缓存中是否已经存在该查询结果
        if not REDIS_CONN.health():
            raise Exception("Lost REDIS connection!")

        cached_results = REDIS_CONN.get(cache_key)
        if cached_results:
            cached_results_dict = json.loads(cached_results)
            sres_dict, sim_list, tsim, vsim_list = cached_results_dict["sres"], cached_results_dict["sim"], cached_results_dict["tsim"], cached_results_dict["vsim"]
            # 将字典列表中的每个元素还原为SearchResult对象
            sres = self.SearchResult.from_dict(sres_dict)
            sim = np.array(sim_list)
            vsim = np.array(vsim_list)
            # print_search_results_and_scores(sres, sim, tsim, vsim, "(来自缓存) ")
        else:
            sres = self.search(req, index_name(tenant_id), embd_mdl, highlight)
            if rerank_mdl:
                sim, tsim, vsim = self.rerank_by_model(rerank_mdl,
                    sres, question, 1 - vector_similarity_weight, vector_similarity_weight)
                print_search_results_and_scores(sres, sim, tsim, vsim)
            else:
                sim, tsim, vsim = self.rerank(
                    sres, question, 1 - vector_similarity_weight, vector_similarity_weight)
                # print_search_results_and_scores(sres, sim, tsim, vsim, "(无重排模型) ")
            cached_results = {'sres':sres.to_dict(), 'sim':sim.tolist(), 'tsim':tsim, 'vsim':vsim.tolist()}
            REDIS_CONN.set_obj(cache_key, cached_results, exp=7200)  # 缓存2小时
        idx = np.argsort(sim * -1)[(page-1)*page_size:page*page_size]

        dim = len(sres.query_vector)
        ranks["total"] = sum(1 for i in sim if i > similarity_threshold)
