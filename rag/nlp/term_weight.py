...
class Dealer:
    def __init__(self):
        # 基础 stopwords（全部小写并转为 set）
        base_stop = set(w.lower() for w in stopwords.words('english'))
        # 你原来示例里有一些缩写/口语 token，可以作为补充
        extras = {
            "d", "ll", "m", "o", "re", "s", "t", "ve", "n't",
            "can't", "cannot", "couldn't", "didn't", "doesn't", "don't", "hadn't", "hasn't", "haven't",
            "he'd", "he'll", "he's", "i'd", "i'll", "i'm", "i've", "let's", "she'd", "she'll", "she's",
            "they'd", "they'll", "they're", "they've", "we'd", "we'll", "we're", "we've", "won't", "wouldn't"
        }

        # 若还有其它常见词也可以加入
        more = {"also", "however", "therefore", "thus", "else", "per", "via", "among", "whilst", "upon"}

        # 最终赋值（和你原来的格式一致）
        self.stop_words = base_stop.union(extras).union(more)
    ...
    def split(self, txt):
        ...
        for t in re.sub(r"[ \t]+", " ", txt).split(" "):
            if tks and re.match(r".*[a-zA-Z]$", tks[-1]) and \
               re.match(r".*[a-zA-Z]$", t) and \
               self.ne.get(t, "") != "func" and self.ne.get(tks[-1], "") != "func" and \
               self.ne.get(t, "") != "loca" and self.ne.get(tks[-1], "") != "loca":
                tks[-1] = tks[-1] + " " + t
            else:
                tks.append(t)
        return tks
    ...
    def weights(self, tks, preprocess=True):
        ...
        def ner(t):
            ...
            if not self.ne or t not in self.ne:
                return 1
            m = {"toxic": 2, "func": 3, "corp": 3, "loca": 3, "sch": 3, "stock": 3,
                 "firstnm": 1}
            return m[self.ne[t]]
