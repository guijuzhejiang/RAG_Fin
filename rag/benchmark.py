...
class Benchmark:
    def __init__(self, kb_id):
        ...
        self.vector_similarity_weight = 0.5
        self.similarity_threshold = 0.2
        self.embd_mdl = LLMBundle(self.kb.tenant_id, LLMType.EMBEDDING, llm_name=self.kb.embd_id, lang=self.kb.language)
        self.embd_mdl.mdl.client._client.base_url = URL('http://127.0.0.1:11434')
        self.tenant_id = ''
        self.index_name = ''
        self.initialized_index = False
        self.rerank_mdl = LLMBundle(self.kb.tenant_id, LLMType.RERANK, 'bge-reranker-v2-m3')
    ...
    def _get_retrieval(self, qrels):
        ...
        for query in query_list:
            ranks = settings.retrievaler.retrieval(query,
                                                   embd_mdl=self.embd_mdl,
    ...
    def init_index(self, vector_size: int):
        if self.initialized_index:
            return
        if settings.ELASTICSEARCH.indexExist(self.index_name):
            settings.ELASTICSEARCH.deleteIdx(self.index_name)
        from conf import conf
        settings.ELASTICSEARCH.createIdx(self.index_name, conf.mapping)
        ...
        for fn in filelist:
            ...
            for i in tqdm(range(len(data)), colour="green", desc="Tokenizing:" + fn):
                ...
                if len(docs) >= 32:
                    ...
                    settings.ELASTICSEARCH.bulk(docs, self.index_name)
                    docs = []
    ...
    def save_results(self, qrels, run, texts, dataset, file_path):
        ...
        for run_i in tqdm(range(len(run_keys)), desc="Calculating ndcg@10 for single query"):
            ...
            keep_result.append({'query': key, 'qrel': qrels[key], 'run': run[key],
                                'result': evaluate({key: qrels[key]}, {key: run[key]}, ["ndcg@10", "map@5", "mrr@10", "precision@10", "recall@10"])})
        keep_result = sorted(keep_result, key=lambda kk: kk['result']['ndcg@10'])
        with open(os.path.join(file_path, dataset + 'result.md'), 'w', encoding='utf-8') as f:
            ...
            for keep_result_i in keep_result:
                f.write('### query: ' + keep_result_i['query'] + ' result:' + str(keep_result_i['result']) + '\n')
    ...
    def __call__(self, dataset, file_path, miracl_corpus=''):
        if dataset == "ms_marco_v1.1":
            ...
            print(dataset, evaluate(Qrels(qrels), Run(run), ["ndcg@10", "map@5", "mrr@10", "precision@10", "recall@10"]))
